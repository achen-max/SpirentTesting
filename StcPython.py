...
STC_PRIVATE_INSTALL_DIR = r"C:\Program Files\Spirent Communications\Spirent TestCenter 5.49\Spirent TestCenter Application"
class StcPython(object):

    def __init__(self):
        self.stcInt = None
        if sys.hexversion < 0x02070EF0 or sys.hexversion > 0x30b02f0 \
        or (sys.hexversion > 0x030404F0 and sys.hexversion < 0x030508F0):
             raise ImportError('This version of STC requires Python version 2.7.14 upto 3.11.2 \
excluding 3.5 variants')
        ...
        elif hex(sys.hexversion).startswith('0x309'):
            self.stcInt = __import__('StcIntPython39')
        elif hex(sys.hexversion).startswith('0x30a'):
            self.stcInt = __import__('StcIntPython310')            
        else:
            self.stcInt = __import__('StcIntPython311')            
